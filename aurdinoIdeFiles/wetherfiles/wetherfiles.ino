...
#define RELAY_PIN 26   // any GPIO except boot pins
...
String lastHallLightPath = "/controls/hallLight";
bool lastKnownHallLight = false;  // default state
...
while (WiFi.status() != WL_CONNECTED && millis() - startAttemptTime < 60000) { // 60-second timeout
  Serial.print(".");
  delay(500);
}
...
if (WiFi.status() == WL_CONNECTED) {
  ...
  // Proceed with initializeApp only if connected
  initializeApp(async_client1, app, getAuth(user_auth), processData, "🔐 authTask");
} else {
  Serial.println();
  Serial.println("Initial WiFi connection timed out. Will retry in loop.");
}
...
if (millis() - lastWifiCheck > wifiCheckInterval) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi disconnected or not connected. Reconnecting...");

    WiFi.disconnect(); // Clean up previous attempt
    WiFi.begin("FTTH-89B3", "12345678");

    unsigned long reconnectStart = millis();
    while (WiFi.status() != WL_CONNECTED && millis() - reconnectStart < 30000) { // 30-second timeout per attempt
      Serial.print(".");
      delay(500);
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      configTime(19800, 0, "pool.ntp.org"); // Resync time

      // Reinitialize Firebase app if not ready
      if (!app.ready()) {
        initializeApp(async_client1, app, getAuth(user_auth), processData, "🔐 authTask");
      }
    } else {
      Serial.println();
      Serial.println("Reconnection timed out. Will try again later.");
    }
  }
}
  ...
  "lastUpdatedAt"
    ...
    if (async_client2.lastError().code() == 0) {
      // Serial.print("LED_PIN, ledControl: ");
      // Serial.println(" %");
      // Serial.print(ledControl ? 'on' : 'off');
      // Serial.println(" %");
      // Serial.print(ledControl);
      digitalWrite(LED_PIN, ledControl ? HIGH : LOW);
    }

     // Read Hall bulb control from Firebase
    bool hallLights = Database.get<bool>(async_client2, "/controls/hallLight");
    if (async_client2.lastError().code() == 0) {
      // Serial.print("LED_PIN, ledControl: ");
      // Serial.println(" %");
      // Serial.print(ledControl ? 'on' : 'off');
      Serial.println(" %");
      Serial.print(hallLights);
      digitalWrite(RELAY_PIN, hallLights ? HIGH : LOW);
    }

      // Read LED control from Firebase
    bool climateControl = Database.get<bool>(async_client2, "/sensor/readNow");
    if (async_client2.lastError().code() == 0) {
      if (climateControl) {
      float temp = dht.readTemperature();
      float hum = dht.readHumidity();
...
long ts = getTimestamp();
      if (!isnan(temp) && !isnan(hum)) {
        ...
        Database.set<float>(async_client1, "/sensor/humidity", hum, processData, "humidity");
        Database.set<float>(async_client1, "/sensor/moisture", moisturePercent, processData, "moisturePercentage");
        Database.set<float>(async_client1, "/sensor/readNow", false, processData, "readNow");
        Database.set<long>(
            async_client1,
            "/sensor/lastUpdatedAt",
            ts,
        ...
        Serial.print(moisturePercent);
        Serial.println(" %");
        Serial.println("updated to fire base");
      }
